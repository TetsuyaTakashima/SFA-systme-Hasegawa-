-- Extend the delegated sales-target creation permission to CSV imports.
-- Non-admin imports cannot change assignment, visibility, record type, or call audit fields on existing rows.
...
begin
  if not (select private.can_create_sales_targets()) then
    raise exception 'You do not have permission to import venues.' using errcode = '42501';
  end if;
  ...
  updated as (
    ...
    set
      ...
      category = i.category,
      record_type = case when is_admin then i.record_type else venue.record_type end,
      operator = i.operator,
      ...
      is_hidden = case when is_admin then i.is_hidden else venue.is_hidden end,
      assigned_user_id = case when is_admin then i.assigned_user_id else venue.assigned_user_id end,
      last_contact_date = i.last_contact_date,
      call_updated_at = case when is_admin then i.call_updated_at else venue.call_updated_at end,
      call_updated_by_user_id = case when is_admin then i.call_updated_by_user_id else venue.call_updated_by_user_id end,
